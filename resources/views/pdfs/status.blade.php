				...
				<table style="width:100%" class="content1">
					<tbody>
						...
						<tr>
							...
							<td nowrap>:</td>
							<td nowrap>{!! App\Yoga::updateDatePrep($periksa->pasien->tanggal_lahir) !!}</td>
						</tr>
						<tr>
							<td nowrap>Pembayaran</td>
							<td nowrap>:</td>
							<td nowrap>{!! $periksa->asuransi->nama_asuransi !!}</td>
						</tr>
					</tbody>
				</table>
			</td>
				...
				<table style="width:100%" class="content1">
					<tbody>
						...
						<tr>
							...
							<td nowrap>:</td>
							<td nowrap>{!! App\Yoga::updateDatePrep($periksa->waktu_datang) !!}</td>
						</tr>
					</tbody>
			</table>
			</td>
		...
		@if($periksa->poli != 'anc' && $periksa->poli !='usg')
			<tr>
				<td class="content2 half">
					...
					{!! $periksa->diagnosa->diagnosa !!} ({!! $periksa->diagnosa->icd_id!!} - 
					{!! $periksa->diagnosa->icd->diagnosaICD !!})
					@if(isset( $periksa->rujukan ))
						...
						{!! $periksa->rujukan->diagnosa->icd->diagnosaICD !!})
					@endif
		...
		@else
			<tr>
				<td colspan="2" class="content1">
					...
					<div class="content1">
						Diagnosa dan diagnosa tambahan : {!! $periksa->diagnosa->diagnosa !!} - {!! $periksa->diagnosa->icd->diagnosaICD !!} <strong>({!!$periksa->diagnosa->icd10_id!!})</strong> <br>
							...
							<td nowrap>
								<table>
									<tr>
										...
										<td nowrap>{!! App\Yoga::updateDatePrep($periksa->registerAnc->registerHamil->hpht) !!}</td>
									</tr>
									<tr>
										<td nowrap class="bold">Umur Kehamilan</td>
										<td nowrap>{!! App\Yoga::umurKehamilan($periksa->registerAnc->registerHamil->hpht, date('Y-m-d')) !!}</td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td nowrap>{!! App\Yoga::datediff($periksa->registerAnc->registerHamil->tanggal_lahir_anak_terakhir, $periksa->tanggal) !!}</td>
									</tr>
									<tr>
										<td nowrap class="bold">Catat di Buku KIA</td>
										<td nowrap>{!!App\Yoga::returnConfirm($periksa->registerAnc->catat_di_kia) !!}</td>
							...
							<td nowrap>
								<table>
									...
									<tr>
										...
										<td nowrap>
											{!! App\Yoga::statusGizi($periksa->registerAnc->lila) !!}
										</td>
									</tr>
									@if($periksa->usg)
									<tr>
